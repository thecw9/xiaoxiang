...
KAFKA_TOPIC = os.environ["KAFKA_REALDATA_TOPIC"]
...
# 网络参数设置
HOST = os.environ["SOUND_SERVICE_HOST"]
...
# 数据接收线程函数
def data_receiving_thread(client_socket, sample_rate, channels, chunk):
    buffer_size = chunk * (sample_rate // chunk) * 60
    while True:
        bytes_needed = buffer_size * channels * 2  # int16 每样本 2 字节
        data_bytes = b""
...
def process_data():
    ...
    while True:
        ...
        if not data_queue.empty():
            ...
            # 保存音频文件、生成波形图、FFT图、梅尔频谱等并上传到 MinIO
            wav_io = BytesIO()
            ...
            save_to_minio(minio_client, BUCKET_NAME, object_name, wav_io, "audio/wav")
...
receiving_thread = threading.Thread(
    target=data_receiving_thread, args=(client_socket, SAMPLE_RATE, CHANNELS, CHUNK)
)
