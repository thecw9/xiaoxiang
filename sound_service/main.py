...
with nidaqmx.Task() as task:
    ...
    # 设置读取位置为最新数据
    task.in_stream.relative_to = const.ReadRelativeTo.MOST_RECENT_SAMPLE
    # task.in_stream.offset = -SAMPLES_PER_CHUNK  # 每次读取最近一块数据
    ...
    try:
        while True:
            ...
            )  # 形状为 (CHANNELS, SAMPLES_PER_CHUNK)
            data_array = (
                data_array - np.mean(data_array, axis=1)[:, np.newaxis]
            )  # 消除直流偏移
