...
import HorizontalComparisonDataTable from "@/components/HorizontalComparisonDataTable.vue";
...
const setComparisonData = async () => {
  ...
  const exclude_info = "告警&增量&测量量&速率&CO2&C2H4";
...
<template>
  ...
  <!-- 数据展示 -->
  <HorizontalComparisonDataTable
    :title="`${form.device}${form.phase}油色谱横向趋势分析`"
    :device-phase="`${form.device}`"
